...
export async function GET(request) {
  ...
  try {
    ...
    const [
      ...
      supabaseAdmin.from('drivers').select('id, name, status, truck_number').eq('company_id', cid),
      supabaseAdmin.from('tickets').select('driver_id, status, created_at, boxes').eq('company_id', cid).gte('created_at', since),
      supabaseAdmin.from('maintenance').select('driver_id, severity, status, created_at').eq('company_id', cid).gte('created_at', since),
      ...
      supabaseAdmin.from('timesheets').select('driver_id, start_time, end_time, state_miles, log_type, date').eq('company_id', cid).gte('date', sinceDate),
    ])
    ...
    const scores = (drivers || []).map(driver => {
      ...
      return {
        ...
        miles: { total: totalMiles, trips },
        safety: {
          score: safetyScore, grade: safetyGrade, hos_violations: hosViolations,
          maintenance_reported: dMaint.length, high_severity: dMaint.filter(m => m.severity === 'high').length,
        },
        pretrip: { completed: pretripCompleted, defects: pretripDefects, defect_rate: defectRate, passed: passedPretrips },
        compliance: { score: complianceScore },
        tickets: { total: totalTickets, approved: approvedTickets, on_time_rate: approvalRate, boxes: totalBoxes },
        productivity: { score: approvalRate },
