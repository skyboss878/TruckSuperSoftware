...
export async function GET(request) {
  ...
  let q = supabaseAdmin.from('driver_documents').select('*, drivers(name)').eq('company_id', ctx.company_id).order('expiry_date', { ascending: true })
...
export async function POST(request) {
  ...
  const { data, error } = await supabaseAdmin.from('driver_documents').insert({ company_id: ctx.company_id,
