...
export default function AdminTracking() {
  ...
  function updateMarkers(data) {
    ...
    data.forEach(trip => {
      ...
      const icon = window.L.divIcon({
        html: `<div style="background:${color};width:36px;height:36px;border-radius:50%;border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;color:white;font-size:16px;">🚛</div>`,
        className: '',
        iconSize: [36, 36],
        iconAnchor: [18, 18],
      })
      ...
      if (markers.current[trip.id]) {
        ...
        markers.current[trip.id].setPopupContent(`<b>${name}</b><br>Truck #${truck}<br>${trip.total_miles?.toFixed(1)} mi<br>Last seen: ${mins}m ago`)
      } else {
        const marker = window.L.marker([trip.last_lat, trip.last_lng], { icon })
          .addTo(leafletMap.current)
          .bindPopup(`<b>${name}</b><br>Truck #${truck}<br>${trip.total_miles?.toFixed(1)} mi<br>Last seen: ${mins}m ago`)
  ...
  function focusDriver(trip) {
    ...
    if (leafletMap.current && trip.last_lat && trip.last_lng) {
      leafletMap.current.setView([trip.last_lat, trip.last_lng], 12)
      ...
      <div className="flex-1 p-4 space-y-3 overflow-y-auto">
        ...
        {!loading && drivers.length === 0 && (
          <div className="text-center py-12 text-gray-400">
            ...
            <p className="text-sm mt-1">Drivers will appear here when they start a trip</p>
          </div>
        )}

        {drivers.map(trip => {
          const mins = trip.last_seen ? Math.floor((Date.now() - new Date(trip.last_seen)) / 60000) : 999
          const statusColor = mins < 5 ? 'bg-green-500' : mins < 15 ? 'bg-orange-500' : 'bg-red-500'
          return (
            <button key={trip.id} onClick={() => focusDriver(trip)}
              className={`w-full bg-white rounded-2xl p-4 shadow-sm text-left active:opacity-70 border-2 ${selected?.id === trip.id ? 'border-[#2D7A5F]' : 'border-transparent'}`}>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-400">Truck #{trip.drivers?.truck_number} · {trip.total_miles?.toFixed(1)} mi</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-gray-400">{lastSeenLabel(trip.last_seen)}</p>
                  {trip.last_lat && (
                    <a href={`https://maps.google.com/?q=${trip.last_lat},${trip.last_lng}`}
                      target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()}
                      className="text-xs text-[#2D7A5F] font-medium">Maps →</a>
                  )}
                </div>
              </div>
            </button>
